...
const invoiceContractPath = path.resolve(
    __dirname,
    "../../contracts/facturatie_contract.xsd",
);
const ALLOWED_TYPES = new Set(["REGULAR", "CREDIT"]);
...
function normalizeEmail(value) {
    ...
    if (!emailRegex.test(normalized) || normalized.length > 254) {
        throw createValidationError("Invalid recipientEmail field");
    }
...
function normalizeInvoiceType(value) {
    ...
    if (!ALLOWED_TYPES.has(normalized)) {
        throw createValidationError(
            `Invalid invoice type '${normalized}'. Allowed values: REGULAR, CREDIT`,
        );
    }
...
function validateWithXmllint(xml) {
    return new Promise((resolve, reject) => {
        const child = spawn(
            "xmllint",
            ["--noout", "--schema", invoiceContractPath, "-"],
            ...
            if (error.code === "ENOENT") {
                reject(
                    createValidationError(
                        "xmllint is required for invoice.finalized XSD validation but was not found",
                    ),
                );
                return;
            }
        ...
        child.on("close", (code) => {
            ...
            reject(
                createValidationError(
                    `invoice.finalized XML failed XSD validation: ${stderr.trim() || "unknown xmllint error"}`,
                ),
            );
        });
...
function createInvoiceFinalizedConsumer({
    mailLogRepository,
    sendgridService,
}) {
    const enabled = parseBoolean(
        process.env.INVOICE_FINALIZED_SYNC_ENABLED,
        true,
    );
    const exchange = process.env.INVOICE_FINALIZED_EXCHANGE || "invoice.topic";
    const exchangeType = process.env.INVOICE_FINALIZED_EXCHANGE_TYPE || "topic";
    const queue =
        process.env.INVOICE_FINALIZED_QUEUE || "mailing.invoice.finalized";
    const routingKey =
        process.env.INVOICE_FINALIZED_ROUTING_KEY || "invoice.finalized";
    const prefetch = Number(process.env.INVOICE_FINALIZED_PREFETCH || 10);
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
            try {
                ...
                if (!Number.isFinite(prefetch) || prefetch <= 0) {
                    throw new Error(
                        "INVOICE_FINALIZED_PREFETCH must be a positive number",
                    );
                }
    ...
    function extractPayload(xmlContent) {
        ...
        return {
            invoiceNumber: normalizeRequiredString(
                payload.invoiceNumber,
                "invoiceNumber",
            ),
            recipientEmail: normalizeEmail(payload.recipientEmail),
            pdfUrl: normalizeRequiredString(payload.pdfUrl, "pdfUrl"),
            totalAmount: normalizeRequiredString(
                payload.totalAmount,
                "totalAmount",
            ),
            type: normalizeInvoiceType(payload.type),
        };
    }
    ...
    async function processMessage(msg) {
        ...
        await processInvoiceFinalized(payload, {
            mailLogRepository,
            sendgridService,
        });
    }
