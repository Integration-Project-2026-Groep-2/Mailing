...
function validateWithXmllint(xml) {
    return new Promise((resolve, reject) => {
            ...
            if (error.code === "ENOENT") {
                reject(
                    createValidationError(
                        "xmllint is required for planning.session.cancelled XSD validation but was not found",
                    ),
                );
                return;
            }
        ...
        child.on("close", (code) => {
            ...
            reject(
                createValidationError(
                    `planning.session.cancelled XML failed XSD validation: ${stderr.trim() || "unknown xmllint error"}`,
                ),
            );
        });
...
function createPlanningSessionCancelledConsumer({
    userRepository,
    mailLogRepository,
    sendgridService,
}) {
    const enabled = parseBoolean(
        process.env.PLANNING_SESSION_CANCELLED_SYNC_ENABLED,
        true,
    );
    const exchange =
        process.env.PLANNING_SESSION_CANCELLED_EXCHANGE || "planning.topic";
    const exchangeType =
        process.env.PLANNING_SESSION_CANCELLED_EXCHANGE_TYPE || "topic";
    const queue =
        process.env.PLANNING_SESSION_CANCELLED_QUEUE ||
        "mailing.planning.session.cancelled";
    const routingKey =
        process.env.PLANNING_SESSION_CANCELLED_ROUTING_KEY ||
        "planning.session.cancelled";
    const prefetch = Number(
        process.env.PLANNING_SESSION_CANCELLED_PREFETCH || 10,
    );
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
            try {
                ...
                if (!Number.isFinite(prefetch) || prefetch <= 0) {
                    throw new Error(
                        "PLANNING_SESSION_CANCELLED_PREFETCH must be a positive number",
                    );
                }
    ...
    function extractPayload(xmlContent) {
        ...
        const payload = parsed?.SessionCancelled;
        if (!payload || typeof payload !== "object") {
            throw createValidationError(
                "Expected root <SessionCancelled> element in payload",
            );
        }
    ...
    async function processMessage(msg) {
        ...
        await processSessionCancelled(payload, {
            userRepository,
            mailLogRepository,
            sendgridService,
        });
