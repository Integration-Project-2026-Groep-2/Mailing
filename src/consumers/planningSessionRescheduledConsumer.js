...
function validateWithXmllint(xml) {
    return new Promise((resolve, reject) => {
            ...
            if (error.code === "ENOENT") {
                reject(
                    createValidationError(
                        "xmllint is required for planning.session.rescheduled XSD validation but was not found",
                    ),
                );
                return;
            }
        ...
        child.on("close", (code) => {
            ...
            reject(
                createValidationError(
                    `planning.session.rescheduled XML failed XSD validation: ${stderr.trim() || "unknown xmllint error"}`,
                ),
            );
        });
...
function createPlanningSessionRescheduledConsumer({
    userRepository,
    mailLogRepository,
    sendgridService,
}) {
    const enabled = parseBoolean(
        process.env.PLANNING_SESSION_RESCHEDULED_SYNC_ENABLED,
        true,
    );
    const exchange =
        process.env.PLANNING_SESSION_RESCHEDULED_EXCHANGE || "planning.topic";
    const exchangeType =
        process.env.PLANNING_SESSION_RESCHEDULED_EXCHANGE_TYPE || "topic";
    const queue =
        process.env.PLANNING_SESSION_RESCHEDULED_QUEUE ||
        "mailing.planning.session.rescheduled";
    const routingKey =
        process.env.PLANNING_SESSION_RESCHEDULED_ROUTING_KEY ||
        "planning.session.rescheduled";
    const prefetch = Number(
        process.env.PLANNING_SESSION_RESCHEDULED_PREFETCH || 10,
    );
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
            try {
                ...
                if (!Number.isFinite(prefetch) || prefetch <= 0) {
                    throw new Error(
                        "PLANNING_SESSION_RESCHEDULED_PREFETCH must be a positive number",
                    );
                }
    ...
    function extractPayload(xmlContent) {
        ...
        const payload = parsed?.SessionRescheduled;
        if (!payload || typeof payload !== "object") {
            throw createValidationError(
                "Expected root <SessionRescheduled> element in payload",
            );
        }
    ...
    async function processMessage(msg) {
        ...
        await processSessionRescheduled(payload, {
            userRepository,
            mailLogRepository,
            sendgridService,
        });
